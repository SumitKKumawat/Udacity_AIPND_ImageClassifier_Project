...
#when inputs for top_k not provided
if top_k is None:
    top_k = 5

#when inputs for device not provided    
if device is None:
    device = "cpu"

#when inputs for category_names not provided
if category_names is None:
    ...
    if file_extension != '.json':
        print("File extension to be .json instead of " + category_names + ".")
        exit()
    else:
...
def loading_model(checkpoint_path):
    ...
    # setting up model as vgg13 for testing purpose
    arch = "vgg13"
    ...
    # building the feed forward network -- vgg model
    ...
    # applying classifier on vgg pretrained network.
    model.classifier = classifier
    model.load_state_dict(check_path['state_dict'])
    return model

#loading the model from saved checkpoints
...
def imshow(image, ax=None, title=None):
    ...
    #no need to add .numpy for image because its an array already.
...
def display_top(image_path, model, save_result_dir):
    ...
    # matplotlib setting up the ploting area
    plt.figure(figsize = (3,6))
    ax = plt.subplot(1,1,1)
    
    # now display the test image of flower
